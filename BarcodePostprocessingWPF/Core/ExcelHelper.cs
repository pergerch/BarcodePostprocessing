                ...
                foreach (string file in files)
                {
                    using (ExcelPackage packageFrom = new ExcelPackage(new FileInfo(file)))
                    {
                        foreach (ResultRow resultRow in resultRows.Where(x => x.Filename == file))
                        {
                            ...
                            {
                                int lastColumn = workSheet.Dimension.End.Column;
                                workSheet.Cells[resultRow.TargetRow, 1, resultRow.TargetRow, lastColumn].Style.Fill
                                    .PatternType = ExcelFillStyle.Solid;
                                workSheet.Cells[resultRow.TargetRow, 1, resultRow.TargetRow, lastColumn].Style.Fill
                                    .BackgroundColor.SetColor(Color.DeepSkyBlue);
            ...
            foreach (string internalCode in internalCodes)
            {
                ...
                switch (rows.Count(x => x.Count != 0))
                {
                    ...
                    default:
                        ...
                        additionalCounts =
                            rows.Where(x => x.Count > 0 && x.Count != null && x.Filename != file)
                                .Select(x => x.Count.Value)
                                .ToList();
