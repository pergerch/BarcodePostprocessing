namespace BarcodePostprocessingWPF
{
	...
	public partial class MainWindow : Window
	{
		...
		private void BtnCompareFiles_OnClick(object sender, RoutedEventArgs e)
		{
			...
			Inventory inventory = ExcelHelper.ReadBarcodeAndCountFromExcelFile(new Inventory(), this.viewModel.RawSummedFileName,
				1, 2, 3);
			...
			int internalCodeColumn =
				Helper.NumberFromExcelColumn(((KeyValuePair<string, string>)this.OfficialInternalCodeColumnBox.SelectedItem).Key);
			int countColumn =
				Helper.NumberFromExcelColumn(((KeyValuePair<string, string>)this.OfficialCountColumnBox.SelectedItem).Key);
			int priceColumn =
				Helper.NumberFromExcelColumn(((KeyValuePair<string, string>)this.OfficialPriceColumnBox.SelectedItem).Key);
			...
			MessageBox.Show(FindResource("ComparisonDoneText").ToString(), FindResource("ComparisonDoneCaption").ToString(),
		...
		private void BtnSumCompareFiles_OnClick(object sender, RoutedEventArgs e)
		{
			...
			{
				MessageBox.Show(FindResource("InputMissingText").ToString(), FindResource("InputMissingCaption").ToString(),
			...
			int internalCodeColumn =
				Helper.NumberFromExcelColumn(((KeyValuePair<string, string>)this.CompareBarcodeColumnBox.SelectedItem).Key);
		...
		private void BtnSumFiles_OnClick(object sender, RoutedEventArgs e)
		{
			...
			int internalCodeColumn =
				Helper.NumberFromExcelColumn(((KeyValuePair<string, string>)this.RawInternalCodeColumnBox.SelectedItem).Key);
			int countColumn =
			...
			{
				inventory = ExcelHelper.ReadBarcodeAndCountFromExcelFile(inventory, item, barcodeColumn, internalCodeColumn,
					countColumn, this.RawFileSkipHeaderCheckbox.IsChecked);
			}
		...
		private void OfficialBarcodeAdd_OnClick(object sender, RoutedEventArgs e)
		{
			...
			{
				KeyValuePair<string, string> item = (KeyValuePair<string, string>)this.OfficialBarcodeColumnBox.SelectedItem;
				if (
					this.OfficialBarcodeList.Items.Cast<KeyValuePair<string, string>>().Any(currentItem => item.Key == currentItem.Key))
				{
